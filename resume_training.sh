...
echo "Loading checkpoint from: ckpts/${CKPT_NAME}/pytorch_model.bin.${Resume_ep-1}"
echo "Loading optimizer state from: ckpts/${CKPT_NAME}/pytorch_opt.bin.${Resume_ep-1}"
...
OMP_NUM_THREADS=48 CUDA_VISIBLE_DEVICES=0,1,2,3 python -m torch.distributed.launch --master_port $RPort --nproc_per_node=4 main_task_retrieval.py --do_train --num_thread_reader=12 \
    ...
    --resume_model ckpts/${CKPT_NAME}/pytorch_opt.bin.${Resume_ep-1} --init_model ckpts/${CKPT_NAME}/pytorch_model.bin.${Resume_ep-1}
